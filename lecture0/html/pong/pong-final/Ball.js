class Ball {
  ...
  bounceHorizontally(hitRatio) {
    const newSpeed = Math.sqrt(this.dx ** 2, this.dy ** 2) * ACCELERATION;
    const direction = -sign(this.dx);
    this.dy = hitRatio;
    this.dx = Math.sqrt(newSpeed - this.dy ** 2) * direction;
  ...
  move(dt, player1, player2) {
    if (this.collides(player1)) {
      this.bounceHorizontally(player1.hitRatio(this.y));
      this.leftEdge = player1.boundary;
    }
    if (this.collides(player2)) {
      this.bounceHorizontally(player2.hitRatio(this.y));
